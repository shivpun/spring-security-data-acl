 ...
 * <p>
 * {@code equals} and {@code hashcode} implementations are based on the {@code Usersname}
 * property only, as the intention is that lookups of the same Users principal object (in a
 * Users registry, for example) will match where the objects represent the same Users, not
 ...
 * {@code UsersDetailsService} each time it is invoked.
 *
 * @author Ben Alex
 * @author Luke Taylor
 */
public class User implements UserDetails, CredentialsContainer {
	 ...
	 */
	public User(Long userId, String usersname, String password,
	 ...
	 * presented the correct Usersname and password and the Users is enabled. Not null.
	 *
	 * @throws IllegalArgumentException if a <code>null</code> value was passed either as
	 * a parameter or as an element in the <code>GrantedAuthority</code> collection
	 */
	public User(Long userId, String usersname, String password, boolean enabled,
	...
	/**
	 * Returns {@code true} if the supplied object is a {@code Users} instance with the
	 * same {@code Usersname} value.
	 * <p>
	 * In other words, the objects are equal if they have the same Usersname, representing
	 * the same principal.
	 */
	@Override
	 ...
	 * improvement to using plain text passwords since the UsersDetails password is
	 * securely hashed. This means if the UsersDetails password is accidentally exposed,
	 ...
	 * </pre>
	 *
	 * @return a UsersBuilder that automatically encodes the password with the default
	 * PasswordEncoder
	...
	public static UsersBuilder withDefaultPasswordEncoder() {
		logger.warn("Users.withDefaultPasswordEncoder() is considered unsafe for production and is only intended for sample applications.");
		PasswordEncoder encoder = PasswordEncoderFactories.createDelegatingPasswordEncoder();
		return builder().passwordEncoder(encoder::encode);
	}

	public static UsersBuilder withUsersDetails(UserDetails UsersDetails) {
		return withUsersname(UsersDetails.getUsername())
			.password(UsersDetails.getPassword())
			.accountExpired(!UsersDetails.isAccountNonExpired())
			.accountLocked(!UsersDetails.isAccountNonLocked())
			.authorities(UsersDetails.getAuthorities())
			.credentialsExpired(!UsersDetails.isCredentialsNonExpired())
			.disabled(!UsersDetails.isEnabled());
	}

	/**
	 * Builds the Users to be added. At minimum the Usersname, password, and authorities
	...
	public static class UsersBuilder {
		 ...
		 * </p>
		 *
		 * @param roles the roles for this Users (i.e. Users, ADMIN, etc). Cannot be null,
		 ...
		 * Populates the authorities. This attribute is required.
		 *
		 * @param authorities the authorities for this Users (i.e. ROLE_Users, ROLE_ADMIN,
		...
		public UserDetails build() {
			...
			return new User(userId, username, encodedPassword, !disabled, !accountExpired,
