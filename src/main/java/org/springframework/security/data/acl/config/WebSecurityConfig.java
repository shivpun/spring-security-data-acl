...
@Configuration
@ConditionalOnBean(value = {AclUserRepository.class})
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		LOGGER.info(
				"Configuration of HttpSecurity \n 1. Enabled SessionFixation. \n 2. Enabled Csrf. \n 3. Enabled Invalidate Session");
		http.sessionManagement().sessionFixation().migrateSession().and().authorizeRequests().anyRequest()
				.authenticated();
		if (!defaultLoginForm) {
			http.formLogin().disable();
		}
		http.cors().and().csrf().and().httpBasic().disable().logout().invalidateHttpSession(true).logoutSuccessUrl("/")
				.clearAuthentication(true).deleteCookies("JSESSIONID", "SESSION");
	}
